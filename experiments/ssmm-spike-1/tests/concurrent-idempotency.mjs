...
const hasConflictCategory = (result, category) =>
  result.status !== 200 && JSON.stringify(result.body).includes(category);
...
const installedShape = (proposalValue, status = "pending") => ({
  ...proposalValue.proposal_content,
  accepted_proposal_id: proposalValue.id,
  accepted_by: "levi",
  accepted_at: "2026-07-31T00:05:00Z",
  installation_actions: status === "pending" ? [] : [{
    description: "Opened workspace",
    evidence: "visible",
    completed_at: "2026-07-31T00:06:00Z",
  }],
...
const semantic = ({ loopId, action, input, expectedRevision, proposalId, proposalVersion }) =>
  normalize({
...
const createProposedFixture = async (suffix) => {
  const opened = await invoke({
    loopId: null,
    eventId: `race-open-${suffix}-${randomUUID()}`,
  ...
  const nextState = {
    ...
    shape_proposals: [candidate],
    proposed_shape: candidate,
  };
  const proposed = await invoke({
    loopId,
    eventId: `race-proposal-${suffix}-${randomUUID()}`,
...
const createAcceptedFixture = async (suffix) => {
  ...
  const acceptedState = {
    ...fixture.state,
    shape_proposals: [{ ...fixture.candidate, proposal_status: "accepted" }],
    proposed_shape: null,
    installed_shape: installedShape(fixture.candidate, "pending"),
  };
  const accepted = await invoke({
    loopId: fixture.loopId,
    eventId: `race-accept-${suffix}-${randomUUID()}`,
    action: "accept_shape_proposal",
    expectedRevision: 2,
    proposalId: fixture.candidate.id,
    proposalVersion: 1,
...
const createMoveFixture = async (suffix) => {
  ...
  const installed = installedShape(fixture.candidate, "installed");
  const nextState = {
    ...fixture.state,
    authoritative_phase: "move",
    current_step: "move_cockpit",
    installed_shape: installed,
    move_custody: moveCustody(),
  };
  const result = await invoke({
    loopId: fixture.loopId,
    eventId: `race-install-${suffix}-${randomUUID()}`,
...
// 1. Different-ID correction and acceptance race from one revision.
{
  ...
  const correction = {
    ...
    nextState: {
      ...fixture.state,
      shape_proposals: [
        { ...fixture.candidate, proposal_status: "superseded" },
        replacement,
      ],
      proposed_shape: replacement,
    },
  };
  const acceptance = {
    loopId: fixture.loopId,
  ...
  const conflicts = results.filter((result) => hasConflictCategory(result, "stale_loop_revision"));
  assert(successes.length === 1, "correction/acceptance race did not produce exactly one success");
  assert(conflicts.length === 1, `correction/acceptance loser was not a revision conflict: ${JSON.stringify(results)}`);
  ...
  const ledgers = await query(
    `runtime_requests?client_event_id=in.(${correction.eventId},${acceptance.eventId})&select=client_event_id`,
  );
  assert(ledgers.length === 1, "correction/acceptance loser created a request ledger row");
  await closeFixture(fixture.loopId);
}

// 2. Different-ID installation confirmations race from one revision.
{
  const fixture = await createAcceptedFixture("installation");
  const installed = installedShape(fixture.candidate, "installed");
  const option = (label) => ({
    loopId: fixture.loopId,
    eventId: `race-install-${label}-${randomUUID()}`,
    action: "confirm_shape_installed",
    expectedRevision: 3,
    proposalId: fixture.candidate.id,
    proposalVersion: 1,
    input: { conditions_satisfied: true, label },
    events: [
      { event_type: "shape_installed", actor: "levi", perspective: "decision", payload: { label } },
      { event_type: "parent_entered_move", actor: "system", perspective: "lr_system_evidence", payload: { label } },
    ],
    nextState: {
      ...fixture.state,
      authoritative_phase: "move",
  ...
  assert(results.filter((result) => result.status === 200).length === 1,
    "installation race did not produce exactly one success");
  assert(results.filter((result) => hasConflictCategory(result, "stale_loop_revision")).length === 1,
    `installation race loser was not a revision conflict: ${JSON.stringify(results)}`);
  const state = await getLoop(fixture.loopId);
  assert(state.authoritative_revision === 4 && state.authoritative_phase === "move",
    "installation race did not produce one coherent Move revision");
  const seamEvents = await query(
    `events?loop_id=eq.${fixture.loopId}&event_type=in.(shape_installed,parent_entered_move)&select=event_type`,
  );
  assert(seamEvents.length === 2, "installation race produced duplicate authoritative seam events");
  await closeFixture(fixture.loopId);
}

// 3. Different-ID Move progress writes race from one revision.
{
  const fixture = await createMoveFixture("progress");
  const option = (label) => ({
    loopId: fixture.loopId,
    eventId: `race-progress-${label}-${randomUUID()}`,
    action: "record_move_progress",
    expectedRevision: 4,
    input: { progress: { label }, move_position: "active" },
    events: [
      { event_type: "move_progress_recorded", actor: "levi", perspective: "ul_levi_report", payload: { label } },
      { event_type: "move_position_updated", actor: "system", perspective: "lr_system_evidence", payload: { move_position: "active" } },
    ],
    nextState: {
      ...fixture.state,
      move_custody: moveCustody({ label }),
    },
  });
  const first = option("first");
  const second = option("second");
  ...
  assert(Boolean(success), "progress race produced no successful request");
  assert(results.filter((result) => hasConflictCategory(result, "stale_loop_revision")).length === 1,
    `progress race loser was not a revision conflict: ${JSON.stringify(results)}`);
  const state = await getLoop(fixture.loopId);
  assert(state.authoritative_revision === 5,
    "progress race incremented revision more than once");
  const progressEvents = await query(
    `events?loop_id=eq.${fixture.loopId}&event_type=eq.move_progress_recorded&select=payload`,
  );
  assert(progressEvents.length === 1,
    "progress race silently persisted both progress writes");
  assert(
    state.move_custody.latest_progress.label === progressEvents[0].payload.label,
    "progress projection does not match the one persisted winner",
  );
  await closeFixture(fixture.loopId);
}
