...
type HumanAuthoritySelfStatus =
  | { status: "ready"; principal_id: "levi"; display_name: "Levi Anthony" }
...
function createHumanAuthorityClient(): SupabaseClient {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const anonKey = process.env.SUPABASE_ANON_KEY;

  if (!getHumanAuthorityConfiguration().configured || !supabaseUrl || !anonKey) {
    throw new Error("HUMAN_AUTHORITY_MISSING_CONFIGURATION");
  }
...
async function authenticateHumanAuthorityClient(): Promise<SupabaseClient> {
  ...
  if (!email || !password) throw new Error("HUMAN_AUTHORITY_MISSING_CONFIGURATION");

  const client = createHumanAuthorityClient();
